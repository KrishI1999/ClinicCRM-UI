    ...
    const fetchVisits = async () => {
      try {
        ...
        const response = await axios.get(
          `https://localhost:5001/api/visits/patient/${id}`,
        ...
        const normalized = data.map((visit) => ({
          ...visit,
          // Unwrap tablets $values
          tablets: visit.tablets?.$values || visit.tablets || [],
          // Unwrap pastIllness $values
      ...
      <div className="max-w-4xl mx-auto space-y-5">
        ...
        {!loading && visits.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">This patient has no recorded visits yet.</p>
          </div>
        )}
        ...
        <div className="space-y-3">
          {visits.map((visit, index) => {
            ...
            return (
              <div key={visit.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-800">Visit #{visit.id}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                  <div className="bg-slate-50 rounded-xl p-3">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Case Details</p>
                    <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">
                      {visit.case || "No case details available"}
                    </p>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-3">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Medical Advice</p>
                ...
                {visit.tablets?.length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Tablets / Medicines</p>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {visit.tablets.map((tablet, i) => (
                        <div key={i} className="flex items-center justify-between bg-violet-50 border border-violet-100 rounded-lg px-3 py-2">
                          <span className="text-sm text-violet-800 font-medium truncate">{tablet.name}</span>
                          <span className="text-xs text-violet-500 flex-shrink-0 ml-2">{tablet.durationDays}d</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {visit.pastIllness?.length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Past Illness</p>
                    <div className="flex flex-wrap gap-2">
                      {visit.pastIllness.map((illness, i) => (
                        <span key={i} className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-medium px-3 py-1 rounded-full">
                          {illness}
                        </span>
                      ))}
                    </div>
                ...
                {visit.referredBy && (
                  <div className="mb-4">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Referred By</p>
                    <p className="text-sm text-gray-700">{visit.referredBy}</p>
                  </div>
                )}
                ...
                {(visit.followUpDate || visit.followUpNotes) && (
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 mb-4">
                    <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide mb-2">Follow-up</p>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3 pt-3 border-t border-gray-50">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm font-medium ${cfg.bg} ${cfg.text} ${cfg.border}`}>
                      ...
                      {visit.payment.pendingAmount > 0 && (
                        <span className="font-normal opacity-75">· ₹{visit.payment.pendingAmount.toFixed(2)} pending</span>
                      )}
                    </div>
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      <span>Billed: <span className="font-semibold text-gray-700">₹{visit.payment.amount?.toFixed(2)}</span></span>
                      <span>Paid: <span className="font-semibold text-emerald-600">₹{visit.payment.paidAmount?.toFixed(2)}</span></span>
                    </div>
                  </div>
                )}
